...
class PropertyDescriptor:
    """Descriptor for a TyConf property with metadata."""

    name: str
    prop_type: type
    default_value: Any
    readonly: bool = False
    validator: Callable[[Any], Any] | None = None  # ✅ Fixed: added type parameters
...
class TyConf:
    ...
    read-only properties, freeze/unfreeze capabilities, and serialization.
    ...
    def __init__(self, **properties: Any) -> None:
        ...
        Args:
            **properties: Keyword arguments where each value is a tuple of:
                         ...
                         - (type, default_value, readonly) - Read-only when readonly=True
        ...
        for name, prop_def in properties.items():
            self._validate_property_definition(name, prop_def)
    ...
    def remove(self, name: str) -> None:
        ...
        try:
            ...
            raise AttributeError(f"Property '{name}' does not exist") from None
    ...
    def copy(self) -> "TyConf":
        ...
        Examples:
            ...
            >>> copy = config.copy()
            >>> copy.frozen  # False
            >>> copy.debug   # False (current value)
            >>> copy.reset()
            >>> copy.debug   # True (original default)
        """
        new_config = TyConf()

        # Copy properties with original defaults and current values
        for name, prop in self._properties.items():
            # Add property with ORIGINAL default value
            new_config.add(
                name=name,
                prop_type=prop.prop_type,
                default_value=prop.default_value,  # Original default
                readonly=prop.readonly,
                validator=prop.validator,
            )

            # Set CURRENT value from source
    ...
    def get(self, name: str, default: Any = None) -> Any:
        ...
        """
        return self._values.get(name, default) if name in self._properties else default
    ...
    def values(self) -> Any:
        ...
        return iter(self._values[name] for name in self._properties)

    def items(self) -> Any:
        """Return an iterator over (name, value) pairs."""
        return iter((name, self._values[name]) for name in self._properties)
    ...
    def from_dict(
        ...
        /,
        *,
        schema: dict[str, tuple[type, Any]] | None = None,  # ✅ Fixed: added type parameters
    ) -> "TyConf":
        """
        ...
        """
        # Detect format: full metadata or values-only
    ...
    def from_env(
        ...
        /,
        *,
        schema: dict[str, tuple[type, Any]],  # ✅ Fixed: added type parameters
    ) -> "TyConf":
        """
        ...
        # Load from environment and update values
        loader = ENVLoader()
    ...
    def _validate_property_definition(self, name: str, prop_def: Any) -> None:
    ...
    def _validate_value(
        ...
        name: str,
        value: Any,
        validator: Callable[[Any], Any],  # ✅ Fixed: added type parameters
    ) -> None:
        """
    ...
    def _format_value_for_display(self, value: Any) -> str:
        ...
        if isinstance(value, str):
            if len(value) > self._MAX_STRING_LENGTH:
                return f"'{value[:self._MAX_STRING_LENGTH - 3]}...'"
            return f"'{value}'"
        ...
        if isinstance(value, (list, tuple)):
            if len(value) > self._MAX_COLLECTION_ITEMS:
                ...
                return f"[{items_str}, ...]" if isinstance(value, list) else f"({items_str}, ...)"
        ...
        if isinstance(value, dict):
            return "{...}" if len(value) > self._MAX_COLLECTION_ITEMS else str(value)
    ...
    def _from_dict_with_metadata(cls, data: dict[str, Any]) -> "TyConf":
        ...
        for name, prop_info in data["properties"].items():
            prop_type = cls._resolve_type(prop_info["type"])
    ...
    def _from_dict_values_only(
        ...
        for name, (prop_type, default_value) in schema.items():
            ...
            # Set actual value if present in data
            if name in data:
                config._values[name] = data[name]
    ...
    def _resolve_type(type_name: str) -> type:
        ...
        type_mapping: dict[str, type] = {  # ✅ Fixed: added explicit type annotation
            "str": str,
            "int": int,
            "float": float,
            "bool": bool,
        ...
        # Fallback to eval for custom types
        # Note: This is safe because type_name comes from serialized data
        return eval(type_name)  # type: ignore[no-any-return]  # ✅ Fixed: suppressed mypy warning
    ...
    def __setattr__(self, name: str, value: Any) -> None:
        ...
        try:
            ...
            raise AttributeError(f"TyConf has no property '{name}'") from None
    ...
    def __str__(self) -> str:
        ...
        props = ", ".join(f"{name}={self._values[name]!r}" for name in sorted(self._properties))
